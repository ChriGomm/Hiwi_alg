...
using .BASEforHANK
using BenchmarkTools, Revise, LinearAlgebra, TimerOutputs
...
timer2 = time()
println("SS solved in $(timer2 - timer1) seconds")
jldsave("Output/Saves/ss250x250x2500.jld2", true; ss)
# @load "Output/Saves/ss80x80x1600.jld2" ss

println("Prepare linearization")
KS = BASEforHANK.prepare_linearization(ss.KSS, ss.VmSS, ss.VkSS, ss.distrSS, ss.n_par, m_par)
@assert(1==0)
