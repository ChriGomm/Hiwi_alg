...
function call_find_steadystate(m_par)
    ...
    KSS, VmSS, VkSS, CDFSS, n_par, m_par = find_steadystate(m_par)

    return SteadyStateStruct(KSS, VmSS, VkSS, CDFSS, n_par)
end

function call_find_steadystate_splines(
    m_par; 
    KSS = nothing, 
    VmSS = nothing, 
    VkSS = nothing, 
    distrSS = nothing)
    #Calculate steady state capital stock
    KSS, VmSS, VkSS, distrSS, n_par, m_par = find_steadystate_splines(
        m_par, 
        K_guess = KSS, 
        Vm_guess = VmSS, 
        Vk_guess = VkSS, 
...
function call_prepare_linearization(ss, m_par)
    ...
    CDF_k,
    distr_y,
    distrSS,m_a_star,k_a_star,m_n_star = prepare_linearization(ss.KSS, ss.VmSS, ss.VkSS, ss.distrSS, ss.n_par, m_par)
...
function compute_steadystate(m_par)
    #Calculate steady state capital stock
    ss = call_find_steadystate_splines(m_par)
